...
```rdf.init(config: RdfConfig): Promise<void>```<br/>
Connects to the database and runs migrations

```rdf.startNotifications(): Promise<void>```<br/>
Starts polling the database for notifications

```rdf.stop(): Promise<void>```<br/>
Stops polling for notifications then disconnects from the database

```rdf.getProjections(): Promise<Projection>[]```<br/>
Returns the list of projections.

```rdf.getProjection(name: string, version: number): Promise<Projection>```<br/>
Returns the specified projection.

```rdf.getChangeLog(projection): Promise<ChangeSet[]>```<br/>
Returns the change log (an ordered list of change sets) for the given projection.

```rdf.getChangeSet(changeSetId): Promise<ChangeSet>```<br/>
