...
const driver = require('./marv-filby-driver');
const { aggregate } = require('./helpers');
...
module.exports = class Filby {
  ...
  async init() {
    ...
    const absoluteDirectoryPermissions = this.#resolveDirectories(relativeDirectoryPermissions);
    await this.#migrateAll([...filbyDirectoryPermissions, ...absoluteDirectoryPermissions]);
  }

  #resolveDirectories(relativeDirectoryPermissions) {
    return relativeDirectoryPermissions.reduce((absoluteDirectoryPermissions, directoryPermissions) => {
      return absoluteDirectoryPermissions.concat({
  ...
  async #migrateAll(directoryPermissions) {
    for (const { path: directory, permissions } of directoryPermissions) {
      ...
      const filbyDriver = driver(this.#config, expandedPermissions);
      await marv.migrate(migrations, filbyDriver);
    }
  }
  ...
  #createScheduler() {
    ...
    return pipsqueak({ name: 'filby', factory, interval, delay: initialDelay });
  }
