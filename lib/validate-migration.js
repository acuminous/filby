...
module.exports = (filename, script) => {
  if (!validate(script)) {
    const message = humanise(filename, validate.errors);
    const err = new Error(message);
    Object.assign(err, { details: validate.errors });
    throw err;
  }
};

function humanise(filename, errors) {
